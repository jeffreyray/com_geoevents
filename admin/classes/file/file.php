...
@define("GEOEVENTS_UPLOAD_EXTENSIONS_JOINED", '');
jimport('joomla.filesystem.file');


/**
* JFile Class for Geoevents.
*
* @package	Geoevents
* @subpackage	Class
*/
class GeoeventsCkClassFile extends JFile
{
	/**
	...
	*/
	public function downloadUrl($path, $dirPattern, $options = array())
	{
		...
		$url = JURI::base(true) . "/index.php?option=com_geoevents&task=file"
			.	"&path=" . $path
			. 	"&action=download";
	...
	*/
	public static function fileBase($file)
	{
		...
		foreach(explode(",", GEOEVENTS_UPLOAD_EXTENSIONS_JOINED) as $joinedExt)
		{
	...
	*/
	public static function getDirectory($path)
	{
		...
		$markers = GeoeventsHelper::getDirectories();

		// TODO : CLEAN DOUBLE CALL
	...
	*/
	public static function getFromIndex($view, $key, $id)
	{
		...
		$model = CkJModel::getInstance($view, 'GeoeventsModel');
		if (!$model)
			return;
	...
	*/
	...
	{
		return GeoeventsHelper::getDirectories();
	...
	*/
	public static function returnFile($mode = null)
	{
		...
		if (!$path)
		{
			...
			//Fallback
			if (!$path)
				$filePath = GEOEVENTS_IMAGES_FALLBACK_ROOT .DS. GEOEVENTS_IMAGES_FALLBACK_NAME;
		}
		...
		{
			require_once(JPATH_ADMIN_GEOEVENTS .DS. 'classes' .DS. 'images.php');
			$thumb = new GeoeventsImages($filePath, $mime);
		...
		else if (!JFile::exists($filePath))
		{
			...
			$msg = JText::sprintf( "GEOEVENTS_UPLOAD_FILE_NOT_FOUND", $path);
...
JLoader::register('GeoeventsClassFile', JPATH_ADMIN_GEOEVENTS .DS. 'fork' .DS. 'classes' .DS. 'file' .DS. 'file.php');
JLoader::load('GeoeventsClassFile'); /* To load class in memory */
// Fallback if no fork has been found
if (!class_exists('GeoeventsClassFile')){ class GeoeventsClassFile extends GeoeventsCkClassFile{} }
