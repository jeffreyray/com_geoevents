...
* @subpackage	Form
*/
class GeoeventsCkClassFormRule extends JFormRule
{
	/**
	...
	*/
	public static function testDefaults(&$element, $value, $group = null, &$input = null, &$form = null)
	{
		...
		if ($unique)
		{
			...
			if ($duplicate)
			{
				JError::raiseWarning(1201, JText::sprintf("GEOEVENTS_MODEL_DUPLICATE_ENTRY_FOR", JText::_($element['label'])));
				return false;
			}
		}
...
JLoader::register('GeoeventsClassFormRule', JPATH_ADMIN_GEOEVENTS .DS. 'fork' .DS. 'classes' .DS. 'form' .DS. 'rule.php');
JLoader::load('GeoeventsClassFormRule'); /* To load class in memory */
// Fallback if no fork has been found
if (!class_exists('GeoeventsClassFormRule')){ class GeoeventsClassFormRule extends GeoeventsCkClassFormRule{} }
