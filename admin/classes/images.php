...
@define("GEOEVENTS_IMAGES_MAX_WIDTH", 1000);
@define("GEOEVENTS_IMAGES_MAX_HEIGHT", 1000);
@define("GEOEVENTS_IMAGES_ALOWED_SIZES", '');
@define("GEOEVENTS_IMAGES_PHYSICAL_THUMB", true);		//Create an hidden thumb file with parameters
@define("GEOEVENTS_IMAGES_FALLBACK_NAME", ".notfound.png");
@define("GEOEVENTS_IMAGES_FALLBACK_ROOT", JPATH_ADMIN_GEOEVENTS .DS. "images");

@include_once(JPATH_ADMIN_GEOEVENTS .DS. "classes" .DS. "file.php");


class GeoeventsImages  extends GeoeventsClassFile
{
	...
	function searchFile(&$create)
	{
		...
		$fallback = $this->dir .DS. GEOEVENTS_IMAGES_FALLBACK_NAME;
		...
		$fallback = GEOEVENTS_IMAGES_FALLBACK_ROOT .DS. GEOEVENTS_IMAGES_FALLBACK_NAME;
		if ($fallback && JFile::exists($fallback))
		{
			$create = 'fly';
			return $fallback;
		...
		switch($create)
		{
			case 'thumb':
				...
				else
				{
					...
					if (($size == "0x0") || in_array($size, explode(",", GEOEVENTS_IMAGES_ALOWED_SIZES)))
					{
						$image = $this->load();
...
*/
    ...
    function writeThumb($image, $name=null)
    {
    	...
    	if (!GEOEVENTS_IMAGES_PHYSICAL_THUMB)
	...
	{
		$this->width = min($width, GEOEVENTS_IMAGES_MAX_WIDTH);
	}

    function height($height='') {
        $this->height = min($height, GEOEVENTS_IMAGES_MAX_HEIGHT);
    }
