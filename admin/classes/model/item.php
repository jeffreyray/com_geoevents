...
* @subpackage	Classes
*/
class GeoeventsCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	public function _upload($fieldName, $extensions = null, $options = array(), $dir = null)
	{
		...
		if (in_array($remove, array('remove', 'delete', 'thumbs', 'trash')))
		{
			...
			if (in_array($remove, array('delete', 'thumbs', 'trash')))
			{
				...
				if (!GeoeventsClassFile::deleteFile($f, $remove)){
					JError::raiseWarning( 4101, JText::_("GEOEVENTS_TASK_RESULT_IMPOSSIBLE_TO_DELETE") );
				}
			}
		}
		...
		switch($method)
		{
			case 'upload':
				...
				// Process Upload
				$uploadClass = new GeoeventsClassFileUpload($dir);
				$uploadClass->setAllowed($extensions);

				if (!$result = $uploadClass->uploadFile($uploadFile, $options))
				{
					JError::raiseWarning( 4100, JText::sprintf("GEOEVENTS_TASK_RESULT_IMPOSSIBLE_TO_UPLOAD_FILE", $uploadFile['name']) );
					return false;
				}
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("GEOEVENTS_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	public function deletefiles($pks, $fileFields)
	{
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!GeoeventsClassFile::deleteFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_GEOEVENTS .DS. 'models' .DS . 'rules');
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		{
			JError::raiseWarning(1106, JText::sprintf("GEOEVENTS_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
		...
		foreach($form->getFieldset($group) as $field)
		{
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format && !GeoeventsHelperDates::isNull((string)$value) )
					{
						$time = GeoeventsHelperDates::getSqlDate($value, array($field->format));
						if ($time === null){
							JError::raiseWarning(1203, JText::sprintf('GEOEVENTS_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
							$valid = false;
						}
						else
							$data[$field->fieldname] = GeoeventsHelperDates::toSql($time);
					}
					break;
...
JLoader::register('GeoeventsClassModelItem', JPATH_ADMIN_GEOEVENTS .DS. 'fork' .DS. 'classes' .DS. 'model' .DS. 'item.php');
JLoader::load('GeoeventsClassModelItem'); /* To load class in memory */
// Fallback if no fork has been found
if (!class_exists('GeoeventsClassModelItem')){ class GeoeventsClassModelItem extends GeoeventsCkClassModelItem{} }
