...
@define("GEOEVENTS_UPLOAD_RANDOM_CHARS", 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789');
@define("GEOEVENTS_UPLOAD_CHMOD_FOLDER", 0744);
@define("GEOEVENTS_UPLOAD_CHMOD_FILE", 0644);


/**
* Uploader Class for Geoevents.
*
* @package	Geoevents
* @subpackage	Class
*/
class GeoeventsCkClassFileUpload extends GeoeventsClassFile
{
	/**
	* Allowed Files types
	*
	...
	*/
	public static function getMaxSize($string = false, $maxSizeCustom = null)
	{
		...
		if ($string)
			$maxSize = JText::sprintf("GEOEVENTS_UPLOAD_MAX_B", self::bytesToString($maxSize));
	...
	*/
	public function process()
	{
		...
		@chmod($fileDest, GEOEVENTS_UPLOAD_CHMOD_FILE);
	...
	*/
	...
	{
		$lenChars = strlen(GEOEVENTS_UPLOAD_RANDOM_CHARS);
		$random = "";
		...
		{
			$pos = rand(0, $lenChars);
			$random .= substr(GEOEVENTS_UPLOAD_RANDOM_CHARS, $pos, 1);
		}
	...
	*/
	public function setUploadFolder($uploadFolder)
	{
		...
		@chmod($uploadPath, GEOEVENTS_UPLOAD_CHMOD_FOLDER);
		if(!is_writable($uploadPath))
		{
			$app->enqueueMessage(JText::sprintf( "GEOEVENTS_UPLOAD_PLEASE_MAKE_SURE_THE_FOLDER_IS_WRITABLE",$uploadPath), 'notice');
			return false;
		}
	...
	*/
	public function uploadFile($uploadFile, $options = array())
	{
		...
		if(empty($uploadFile['name'])){
			$app->enqueueMessage(JText::_("GEOEVENTS_UPLOAD_PLEASE_BROWSE_A_FILE"),'notice');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::sprintf( "GEOEVENTS_UPLOAD_THIS_FILE_EXTENSION_IS_NOT_ACCEPTED_THE_ACCEPTED_FILES_ARE",
												$file->extension,
		...
		{
			$app->enqueueMessage(JText::sprintf( "GEOEVENTS_UPLOAD_MIME_TYPE_NOT_VALID_ALLOWED_MIMES_ARE",
												$this->file->mime,
		...
		{
			$app->enqueueMessage(JText::sprintf( "GEOEVENTS_UPLOAD_TOO_BIG_FILE_BYTES_MAX_ALLOWED_SIZE_BYTES",
		...
		{
			$app->enqueueMessage(JText::_( "GEOEVENTS_UPLOAD_THE_FILE_CONTAINS_ERRORS"), 'error');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::sprintf( "GEOEVENTS_UPLOAD_THIS_FILE_ALREADY_EXIST",$file->filename), 'notice');
			return false;
		}
		...
		{
			if ($app->isSite())
				$msg = JText::sprintf( "GEOEVENTS_UPLOAD_COULD_NOT_UPLOAD_THE_FILE", $file->filename);	// Don't show the complete directory in front-end
			else if ($app->isAdmin())
				$msg = JText::sprintf( "GEOEVENTS_UPLOAD_COULD_NOT_UPLOAD_THE_FILE_TO",$file->tmp,$this->uploadFolder . $file->filename);
...
JLoader::register('GeoeventsClassFileUpload', JPATH_ADMIN_GEOEVENTS .DS. 'fork' .DS. 'classes' .DS. 'file' .DS. 'upload.php');
JLoader::load('GeoeventsClassFileUpload'); /* To load class in memory */
// Fallback if no fork has been found
if (!class_exists('GeoeventsClassFileUpload')){ class GeoeventsClassFileUpload extends GeoeventsCkClassFileUpload{} }
